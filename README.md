...
2. **角色专属工位** —— 9 个 AI 角色各有独立工作站（架构、前端、后端、产品、项目经理、测试、资讯、日报、安全），Agent 自动出现在对应角色区域，支持点击查看角色详情
3. **昨日小记** —— 自动从 `memory/*.md` 读取最近一天的工作记录，脱敏后展示为"昨日小记"卡片
4. **多 Agent 协作** —— 通过 join key 邀请其他 Agent 加入你的办公室，实时查看多人状态
5. **中英日三语** —— CN / EN / JP 一键切换，界面文案、气泡、加载提示全部联动
6. **美术资产自定义** —— 侧边栏管理角色 / 场景 / 装饰素材，支持动态帧同步，避免闪烁
7. **AI 生图装修** —— 接入 Gemini API，用 AI 给办公室换背景；不接入 API 也能正常使用核心功能
8. **移动端适配** —— 手机直接打开即可查看，适合外出时快速瞄一眼
9. **安全加固** —— 侧边栏密码保护、生产环境弱密码拦截、Session Cookie 加固
10. **灵活公网访问** —— 推荐 Cloudflare Tunnel 一键公网化，也可用自有域名 / 反向代理
11. **桌面宠物版** —— 可选的 Tauri 桌面封装，把办公室变成透明窗口的桌面宠物（见下方说明）
...
办公室为 9 个标准 AI 角色预留了专属工作站，Agent 会根据 `agentId` 自动被分配到对应工位：

| 角色 ID | 工位名称 | 区域位置 | 特点 |
|---------|---------|---------|------|
| `architect` | 🏗️ 架构工作站 | 左上方 | 系统设计与架构规划 |
| `frontend-dev` | 🎨 前端工作室 | 上方左中 | UI/UX 实现 |
| `backend-dev` | ⚙️ 后端实验室 | 上方中央 | 服务端开发 |
| `product-manager` | 📋 产品经理区 | 右中方 | 需求与规划 |
| `project-manager` | 📊 项目看板区 | 右中下 | 项目管理与协调 |
| `qa-engineer` | 🧪 测试质检区 | 右上方 | 质量检验与测试 |
| `news-miner` | 📰 资讯研究室 | 右下方 | 信息采集与分析 |
| `daily-reporter` | 📝 日报助手站 | 右下方中 | 日报生成与编辑 |
| `security-expert` | 🔒 安全监控室 | 右下角 | 安全评估与防护 |

**交互方式：**
- Agent Sprite 自动出现在该角色的工位区域
- 点击工位标签或左侧 Agent 卡片 → 弹窗显示角色详情（状态、Token 消耗、会话数）
- 鼠标悬停工位 → 蓝色高亮边框提示可交互

**坐标自定义：**
工位坐标定义在 `frontend/index.html` L3570 的 `ROLE_ZONE_RECTS` 常量中，可根据背景图布局需要调整。使用"显示坐标"调试按钮可实时查看鼠标位置。
...
| 日期 | 概要 | 详情 |
|------|------|------|
| 2026-04-06 | 🏢 角色工位系统 — 9 个标准 AI 角色各配备专属工作站，Agent 自动出现在对应工位，支持弹窗详情和交互高亮 | 新功能发布 |
...
│   └── run.sh
├── frontend/           # 前端页面与资产
│   ├── index.html              # Phaser 3 主游戏（包含工位系统）
│   ├── workstations.js         # 角色工位交互系统
│   ├── join.html
│   ├── invite.html
│   ├── openclaw-panel.js       # Agent 卡片与工位交互
│   ├── stats-panel.js          # 数据统计与排行榜
│   └── ... 其他 UI 模块
